...
<body>
	...
	<button type="button" onclick="location.href='${pageContext.request.contextPath}/product/productList'" >되돌아가기</button>
	<button type="button" onclick="location.href='managementRegister_Form?productNo=${param.productNo }'" >상품 재고 등록</button>
	<hr>
	<table border="1">
		<tr>
			<th>보유 사이즈</th> <th>보유 색</th> <th>남은 재고 수량</th> <th>재고 관리</th>
		</tr>
		<c:forEach var="mdto" items="${list }">
			<tr>
				<td>${mdto.productSize}</td> <td>${mdto.productColor }</td> <td>${mdto.productStack }</td>
				<td>
